...
import com.lightstreamer.kafka.examples.quick_start.producer.FeedSimulator.Position;
...
public class Producer implements Runnable, FeedSimulator.ExternalFeedListener {
    ...
    private KafkaProducer<String, Position> producer;
    ...
    public void onEvent(Position position) {
        System.out.printf("Publishing position: %s%n", position);
        ProducerRecord<String, Position> record =
                new ProducerRecord<>(this.topic, position.positionId(), position);
        record.headers().add("account_id", position.accountId().getBytes());
        try {
            producer.send(
                    record,
                    new Callback() {
