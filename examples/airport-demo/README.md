...
This project includes the resources needed to develop the Lightstreamer Airport Demo pluggable into Lightstreamer Server and leveraging the [Lightstreamer Kafka Connector](https://github.com/Lightstreamer/Lightstreamer-kafka-connector).
...
The simulated data, inputted into a [Kafka cluster](https://kafka.apache.org/), is fetched and injected into the Lightstreamer server via the [Lightstreamer Kafka Connector](https://github.com/Lightstreamer/Lightstreamer-kafka-connector).
...
The source code of the producer is basically contained in the `producer` package, that generates random information for the flights and acts as the producer versus the Kafka cluster. In particular the following classes are defined:
...
In the `connector` folder we found the configuration files needed to configure the Lightstreamer Kafka Connector:
...
You can easily build the jar using Maven through the `pom.xml` file located in the  `producer` folder. As an alternative, you can use any other alternative build tool (e.g. Gradle, Ivy, etc.).
