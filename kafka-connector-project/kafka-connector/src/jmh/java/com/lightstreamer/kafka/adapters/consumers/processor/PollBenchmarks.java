...
public class PollBenchmarks {
    ...
    private static final Logger log = LoggerFactory.getLogger(PollBenchmarks.class);
    ...
    public static class Protobuf {
        ...
        private Deserializer<DynamicMessage> deserializer;

        private RecordMapper<String, DynamicMessage> recordMapper;
        ...
        public void setUp() {
            ...
            // Configure the RecordMapper.
            this.recordMapper = BenchmarksUtils.newRecordMapper(config);
            ...
            this.deserializer =
                    DynamicMessageDeserializers.ValueDeserializer(configurator.getConfig());
            // Generate the test records.
    ...
    @Benchmark
    public void pollProtoDeferred(Blackhole bh, Protobuf proto) {
        ConsumerRecords<Deferred<String>, Deferred<DynamicMessage>> deferredRecords =
                ProtoRecords.deferredConsumerRecords(proto.rawRecords, proto.deserializer);

        // bh.consume(deferredRecords);
        // proto.recordConsumer.consumeRecords(KafkaRecords.from(deferredRecords));
        KafkaRecords<String, DynamicMessage> records = KafkaRecords.from(deferredRecords);
        for (KafkaRecord<String, DynamicMessage> r : records) {
            MappedRecord mp = proto.recordMapper.map(r);
            bh.consume(mp);
        }
    }

    @Benchmark
    public void pollProto(Blackhole bh, Protobuf proto) {
        ConsumerRecords<Deferred<String>, Deferred<DynamicMessage>> consumerRecords =
                ProtoRecords.resolvedConsumerRecords(proto.rawRecords, proto.deserializer);
        // bh.consume(consumerRecords);
        // proto.recordConsumer.consumeRecords(KafkaRecords.from(consumerRecords));
        KafkaRecords<String, DynamicMessage> records = KafkaRecords.from(consumerRecords);
        for (KafkaRecord<String, DynamicMessage> r : records) {
            MappedRecord mp = proto.recordMapper.map(r);
            bh.consume(mp);
        }
    ...
    public static void main1(String[] args) throws Exception {
        Options opt =
                new OptionsBuilder()
                        ...
                        .include(PollBenchmarks.class.getSimpleName())
                        .build();
