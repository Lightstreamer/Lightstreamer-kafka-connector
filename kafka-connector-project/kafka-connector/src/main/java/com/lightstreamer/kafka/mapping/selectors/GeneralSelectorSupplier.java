...
import static com.lightstreamer.kafka.mapping.selectors.ExpressionException.expectedRootToken;
...
public class GeneralSelectorSupplier implements SelectorSupplier<GeneralSelector> {
    ...
    @Override
    public GeneralSelector newSelector(String name, String expression) {
        Constant constant = Constant.from(expression);
        if (constant == null) {
            throw expectedRootToken(name, expectedConstantStr());
        }
