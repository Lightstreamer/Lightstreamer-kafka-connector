...
public class KafkaConnectorAdapter implements SmartDataProvider {
  ...
  private <K, V> Loop loop(ConsumerLoopConfig<K, V> config, ItemEventListener eventListener) {
    MetadataListener metadataAdapter =
        KafkaMetadataAdapter.listener(connectorConfig.getText(ConnectorConfig.DATA_ADAPTER_NAME));
  ...
  public void subscribe(@Nonnull String itemName, @Nonnull Object itemHandle, boolean needsIterator)
    ...
    if (itemName.equals(connectorConfig.getText(ConnectorConfig.ITEM_INFO_NAME))) {
      ...
      loop.subscribeInfoItem(
          new InfoItem(itemHandle, connectorConfig.getText(ConnectorConfig.ITEM_INFO_FIELD)));
    } else {
