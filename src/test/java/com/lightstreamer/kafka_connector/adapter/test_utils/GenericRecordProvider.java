...
public class GenericRecordProvider {
  ...
  private GenericRecord newGenericRecord() {
    ...
    parentJoe.put(
        "preferences", Map.of(new Utf8("pref1"), "pref_value1", new Utf8("pref2"), "pref_value2"));
    Schema enumSchema = valueSchema.getField("type").schema();
    parentJoe.put("type", new GenericData.EnumSymbol(enumSchema, "TYPE1"));

    parentJoe.put(
        "signature",
        new GenericData.Fixed(valueSchema.getField("signature").schema(), "abcd".getBytes()));
    ...
    childAnna.put(
        "children", new GenericData.Array<>(childrenSchema, List.of(childGloria, childTerence)));
